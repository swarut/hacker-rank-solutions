defmodule Solution do
  def solve(scores, alice_scores) do
    ...
    Enum.reduce(alice_scores, %{ranks: [], last_rank: last_rank}, fn(score, acc) ->
      current_rank = cond do
        acc[:last_rank] > (last_rank + 1) -> last_rank
        true -> acc[:last_rank]
      end
      rank = compute_rank(lookup, score, 1, current_rank)
    ...
    #Enum.reduce(alice_scores, %{ranks: []}, fn(score, acc) ->
    #  rank = compute_rank(lookup, score, 1, last_rank)
    #  Map.replace(acc, :ranks, acc[:ranks] ++ [rank])
    #end)
